...
//		print2DArray(w);
	...
	void trainOrTestNet(boolean train, int maxCount, MNISTReader frame) {
		...
		for (int count = 1; count < maxCount; count++) {
			...
			for (int t = 0; t < NEURONS - 10; t++) {
				input[t] = trainImage[pattern % 100][t]; // initialize original
															// pattern
			}
	 ...
	 * 
	 * try { fos = new FileOutputStream(fileName); fos.write(buf); }
	 * catch(IOException ex) { System.out.println(ex); } finally { if(fos!=null)
